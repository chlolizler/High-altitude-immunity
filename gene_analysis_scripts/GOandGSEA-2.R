d <- read.csv("~/Documents/Mentees/Chloe Butler/MergedDE_ortho_082825.csv", header = TRUE)

d$PopManualPvalAdj <- 1
d$PopManualPvalAdj[which(d$Agreement==1)] <- p.adjust(d$P.Value.Pop[which(d$Agreement==1)], method = "BH", n = 314)

d$InjManualPvalAdj <- 1
d$InjManualPvalAdj[which(d$Agreement==1)] <- p.adjust(d$P.Value.Inj[which(d$Agreement==1)], method = "BH", n = 314)

d$IxnManualPvalAdj <- 1
d$IxnManualPvalAdj[which(d$Agreement==1)] <- p.adjust(d$P.Value.PopByInj[which(d$Agreement==1)], method = "BH", n = 314)

write.csv(d, "~/Documents/Mentees/Chloe Butler/MergedDE_ortho_pManual_082825.csv")
...
require(DOSE)
dotplot(gse_Pop, showCategory=10, split=".sign") + facet_grid(.~.sign)
dotplot(gse_LPS, showCategory=10, split=".sign") + facet_grid(.~.sign)
dotplot(gse_Ixn, showCategory=10, split=".sign") + facet_grid(.~.sign)
